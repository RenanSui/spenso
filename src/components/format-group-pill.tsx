...
export default function FormatGroupPill({
  ...
  const { revenue, expenses } = calculatePills(transactions, rates, currency)
  const isRevenueGreater = revenue.value > toPositive(expenses.value)
  ...
  return mounted ? (
    ...
    >
      ...
      {isRevenueGreater
        ? `${formatValue(revenue.value, currency)} revenue`
        : `${formatValue(expenses.value, currency)} expenses`}
    </GroupPill>
  ) : (
