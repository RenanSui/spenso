...
import { cn, positiveOrNegative } from '@/lib/utils'
...
export const GuestTransactionForm = ({
  setOpen,
  groupId,
  formAction,
  transaction,
  ...
  const processForm: SubmitHandler<z.infer<typeof formSchema>> = async (values: z.infer<typeof formSchema>) => {
    ...
    // process form add
    if (formAction === 'add') {
      guest.createTransaction({
        ...newValues,
        amount: positiveOrNegative(newValues.type, newValues.amount),
        year: new Date(newValues.date).getFullYear().toString(),
