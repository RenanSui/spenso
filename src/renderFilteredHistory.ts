...
export const renderFilteredHistory = (action: string) => {
    ...
    if(action === 'Balance') newHistory = currentHistory.filter( (item) =>  item)
    if(action === 'Income') newHistory = currentHistory.filter( (item) =>  item.type === 'Income')
    if(action === 'Expense') newHistory = currentHistory.filter( (item) =>  item.type === 'Expense')
